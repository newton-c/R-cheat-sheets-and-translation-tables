...
NewDF <- merge(firstDF, secondDF, by=c("variable 1", "variable 2"), all=TRUE)
...
df <- df[!(df$variable == 'value'),] # if variable = 'value' the row is dropped
...
df$ccode1 <- countrycode::countrycode(sourcevar = df$varname,
   origin = "country.name", destination = "cown", custom_match = 
   c("Serbia" = "345", "Central African Rep." = "483")) 
#decting duplicates
...
# pretty graph
library(ggplot2)
ggplot(df, aes(x=df$x, y=df$y)) + geom_point(aes(color=df$z),alpha = 0.5,
  size=df$z) + scale_colour_gradient(low = "blue", high = "red") + geom_line() + 
...
df_b <- with(df_a, data.frame(x1 = 0, x2 = 0, x3 = c(-.1, 0, .1), x4 = mean(x4)))
...
library(plm)
df <- pdata.frame(df, index = c("cross_section_var","time_series_var")) # set df to panel
